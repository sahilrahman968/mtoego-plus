    ...
    <div ref={containerRef} className="relative">
      <div className="flex">
        <button
          ...
          onClick={() => setOpen((p) => !p)}
          disabled={disabled}
          className="flex items-center gap-1.5 px-3 py-2.5 text-sm border border-border border-r-0 rounded-l-lg bg-card hover:bg-card-hover transition-colors shrink-0 disabled:opacity-50"
          aria-label="Select country code (WhatsApp OTP)"
        >
          ...
          <span className="text-foreground/80">{selectedCountry.dial}</span>
          <ChevronDown
            size={12}
      ...
      {open && (
        <div className="absolute z-50 top-full left-0 mt-1 w-72 bg-card border border-border rounded-lg shadow-xl shadow-black/40 overflow-hidden">
